...
export default function AdminPage() {
  ...
  function fetchData() {
    ...
    Promise.all([
    ...
    ]).then(async ([r, m, pRes, pollsRes]) => {
      if (r.data) setRalphDrinks((r.data as { ralph_drink_count: number }).ralph_drink_count);
      setMessages((m.data || []) as Message[]);

      const postsData = (pRes.data || []) as { id: string; guest_id: string; message: string; photo_urls: string[]; created_at: string }[];
      if (postsData.length > 0) {
        ...
        const pollsData = (pollsRes?.data || []) as { id: string; guest_id: string; question: string; created_at: string }[];
  ...
  useEffect(() => {
    ...
    supabase
      .from("guests")
      .upsert(
        { guest_id: adminGuestId, name: "Host", updated_at: new Date().toISOString() },
        { onConflict: "guest_id" }
      )
      .then(() => {
        supabase
          ...
          .then(({ data }) => {
            if (data) {
              const d = data as { name: string; avatar_url: string | null };
              setHostName(d.name || "Host");
              setHostAvatar(d.avatar_url);
            }
            fetchData();
          });
      });
